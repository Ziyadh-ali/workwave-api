...
export class MeetingUseCase implements IMeetingUseCase {
    ...
    async createMeeting(meeting: IMeeting, filter: { role?: string; department?: string }): Promise<MeetingResponseDTO> {
        ...
        const participants = await this._employeeRepository.getParticipantsByFilter(filter);
        ...
        for (const emp of participants) {
            const employee = await this._employeeRepository.findById(emp.toString());
            if (employee) {
        ...
        const hostMeetings = await this._meetingRepository.getMeetingsByHost(
        ...
        const allEmployeeMeetings = await Promise.all(
            filteredParticipants.map(p => this._meetingRepository.getMeetingsByEmployeeId(p))
        );
        ...
        return MeetingMapper.toResponseDTO(await this._meetingRepository.create(meeting));
    }

    async deleteMeeting(id: string): Promise<void> {
        await this._meetingRepository.delete(id);
    }

    async getMeetingByDate(date: Date): Promise<MeetingResponseDTO[]> {
        return (await (this._meetingRepository.getMeetingByDate(date))).map(MeetingMapper.toResponseDTO);
    }
    ...
    async getMeetingsByHost(hostId: string): Promise<MeetingResponseDTO[]> {
        return (await this._meetingRepository.getMeetingsByHost(hostId)).map(MeetingMapper.toResponseDTO);
    }

    async updateMeeting(id: string, updateData: Partial<IMeeting>): Promise<MeetingResponseDTO | null> {
        const meeting = await this._meetingRepository.update(id, updateData);
        return meeting ? MeetingMapper.toResponseDTO(meeting) : null;
    }
    ...
    async editMeeting(meeting: IMeeting, filter: { role?: string; department?: string }): Promise<MeetingResponseDTO |  null> {
        ...
        const hostMeetings = await this._meetingRepository.getMeetingsByHost(meeting.createdBy ? meeting.createdBy.toString(): "");
        const isHostOverlap = hostMeetings.some(existing => {
        ...
        const data =  await this._meetingRepository.update(meeting._id ? meeting._id.toString() : "", meeting);
    ...
    async getMeetingByEmployeeId(employeeId: string): Promise<MeetingResponseWithParticipantsDTO[]> {
        const meetings = await this._meetingRepository.getMeetingsByEmployeeId(employeeId);
